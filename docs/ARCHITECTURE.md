...
On Unix, foreground Bash commands and child `zdx exec` runs use `zdx-tools::process_supervisor`. The owner retains the sole lease endpoint while an external supervisor, isolated from both the owner's and target's process groups, directly owns the target. Lease closure from cancellation, future drop, or owner-process death triggers TERM, a bounded teardown grace, KILL if needed, same-group cleanup, and target reaping. Healthy commands have no supervision deadline. `WorkerManager` owns worker-prompt leases independently of originating chat turns, while Bash `background: true` continues through the durable detached-process registry and intentionally bypasses invocation supervision. Descendants that deliberately leave the owned process group are outside this containment model.
