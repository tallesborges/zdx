...
`zdx-bot/src/server.rs` projects raw RFC3339 timestamps and optional request/tool boundaries through the existing read-only thread endpoint. `apps/web/src/lib/trajectory.ts` groups that flat stream into user turns, pairs tools in request order, packs overlapping exact spans into tracks, and computes active wall time as an interval union rather than a duration sum. The mobile-first Trajectory pane keeps lane labels pinned inside a horizontal time canvas, ranks all recorded durations in a separate bottleneck view, and opens payload/result timing in a bottom sheet. Events without explicit boundaries stay unanchored; their durations may be shown and ranked, but the client never invents overlap for older threads.
