...
- **zdx-tools:** Leaf tool implementations (bash, edit, read, write, glob, grep, etc.) with a minimal ToolContext, plus Unix supervision for invocation-owned process groups. Depends on zdx-types.
...
- **Event-driven coordination only:** the orchestrator delegates, ends its conversational turn, and is resumed by the worker's completion callback. The manager exposes no call that blocks on worker progress, so there is no wait registry, no claiming of completions by a waiter, and no suppression or replay of callbacks — every finished prompt emits exactly one `WorkerEvent::Completed` and the bridge dispatches it. `Get_Thread_Status` is on-demand inspection of current state, not a polling primitive. The one bounded wait left is `wait_for_mirror_url`: it waits ≤5s on the mirror-resolution signal for `create_thread` to hand back a topic link, and is unrelated to worker progress — the worker is already queued and never waits on it.
...
- **Child-runner reuse:** worker prompts run through `run_exec_subagent_with_cancel` with `ExecSubagentOptions.thread_id`, i.e. `zdx --thread <id> exec` in the worker's project root, so the worker resumes its own JSONL history. The manager-owned runner holds the child supervisor lease, so workers survive the originating orchestrator turn but cancellation or manager-process death closes the lease and completes TERM→KILL cleanup before the FIFO can start a successor.
...
- **Completion bridge (`zdx-bot/src/orchestrator.rs`):** one task consumes the manager's `WorkerEvent` stream (per worker: `Created` → `Prompted`/`Activity`* → `Completed`). `Created` opens the Telegram mirror topic and registers its link back on the manager (`set_mirror_url`, resolved even when no mirror opened), which is how the asynchronous topic reaches `create_thread`'s bounded mirror wait and every later snapshot/callback. `Activity` (the child runner's `SubagentStreamSink` relayed as `WorkerActivity`) drives one debounced live message per turn. Every `Completed` carries just its `CompletionEvent`: the bridge finishes the live turn, posts the mirror update, and unconditionally dispatches one synthetic `[worker update]` message through the owner topic's normal per-topic queue, using the same shared synthetic-message helper (`zdx-bot/src/bot/synthetic.rs`) as `/goal` continuations. Routes are recorded per orchestrator turn and are process-lifetime; the worker→mirror map is rebuilt at startup from persisted `worker_topic` + `alias_to` meta lines (`list_worker_topics`), so no extra store is needed; missed callbacks are dropped by design.

## Owned Process Lifetimes

On Unix, foreground Bash commands and child `zdx exec` runs use `zdx-tools::process_supervisor`. The owner retains the sole lease endpoint while an external supervisor, isolated from both the owner's and target's process groups, directly owns the target. The supervisor reports the target's `pid`/`pgid` back over the startup handshake, so the owner can register the process without inspecting it. Lease closure from cancellation, future drop, or owner-process death triggers TERM, a bounded teardown grace, KILL if needed, same-group cleanup, and target reaping. Healthy commands have no supervision deadline: the foreground bound (`bash_foreground_bound_secs`) ends the *wait*, not the command. On expiry the live handle — lease, completion, pipes, process group — moves out of the tool call into the process-global adopted-jobs registry (`zdx-tools::adopted`), the stream sinks flip from the in-memory tail to the background log files, and the job continues under the same supervisor; this makes adopted jobs session-scoped and killable through the same lease path. Because the adopting process holds that lease, the handoff is enabled only where the process can keep the job alive, decided by `tools::surface_keeps_background_jobs`: the TUI (`chat`) and the bot daemon (`telegram`) outlive the run, while `zdx exec` — and therefore every `invoke_subagent` child and orchestrator worker — *drains* instead, awaiting `zdx_tools::adopted::drain` at the end of the exec command before returning from `dispatch`. Draining must complete inside `rt.block_on`: the runtime is dropped when `cli::run` returns, which would drop the task holding the lease and kill the job. `adopt` awaits the job's reader tasks and records its exit before deregistering it, so a drained process never truncates a job's log or leaves its bookkeeping half-written. An interrupt during the drain routes to `terminate_all`, closing every lease so nothing is orphaned. `WorkerManager` owns worker-prompt leases independently of originating chat turns, while Bash `background: true` continues through the durable detached-process registry and intentionally bypasses invocation supervision. Descendants that deliberately leave the owned process group are outside this containment model.
...
### Usage Accounting
Token usage is event-sourced. The agent buffers usage deltas per attempt and emits one combined `AgentEvent::UsageUpdate` (carrying the active `model` + `provider`) at commit boundaries; transparently retried attempts drop their buffered usage to avoid double counting. A successful request's terminal usage event (the `consume_stream` EOF-success flush) additionally carries per-request latency (`duration_ms` + `ttft_ms`) and explicit wall-clock boundaries (`started_at` + `completed_at`); interim/failed flushes do not, so timing rides exactly one usage event per request. `UsagePersistor` (in `thread_persistence`) turns those events into `usage` `ThreadEvent`s, attaching the model/provider and terminal timing so any saved thread can be attributed and placed on a trajectory. Tool completions carry the same optional boundaries; the persistor queues timing metadata by tool id until checkpoint-batched results are written in canonical request order, preserving concurrent overlap without changing replay order. `core/usage_stats.rs` aggregates usage/cost across all saved threads (per provider/model) for `zdx stats` and the monitor, reusing `ModelPricing` for cost. To stay fast at thousands of threads it maintains a **derived, disposable SQLite cache** (`$ZDX_HOME/cache/usage.sqlite`) of each thread's partial aggregate keyed by `(thread_id, mtime, size)`, re-scanning only changed threads (JSONL stays canonical; the cache is rebuilt on schema/`default_model` change or corruption, and bypassed via a full lean scan if unavailable). The monitor runs the aggregation on a worker thread so the dashboard never blocks.

### Shared Thread Trajectory

`zdx-engine/src/core/thread_trajectory.rs` is the canonical, UI-agnostic projection from persisted `ThreadEvent`s plus optional live tool snapshots into per-turn spans, exact interval tracks, observed wall/span/concurrency aggregates, and bottleneck rankings. It preserves recorded request/tool duration separately from explicit boundary width, pairs repeated tool IDs FIFO, and keeps legacy or incomplete spans unanchored. `zdx threads inspect --json` exposes that serde model for agents/scripts; the text CLI and Monitor timing overlay render its shared formatter. The authenticated Mini App endpoint `GET /api/threads/{id}/trajectory` adds bounded live-tool snapshots and returns the same projection. The Svelte pane owns only the mobile timeline, bottleneck, and detail presentation, joining persisted payload/result details from the existing activity stream by source-event sequence.
