...
- The orchestrator's reply automatically ends with one `🛠 <worker title>` link per worker the turn created or messaged (successful `Create_Thread`/`Send_Thread_Message` results, first-touch order, placed above `↗ Open thread`), so the user reaches the mirror without relying on the model to link it. Workers without a linkable mirror are skipped.
