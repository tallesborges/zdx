...
- Native memory docids use the disjoint grammar `zdxmem:v1:<source>:<hex16>`, where `<source>` is `thread`, `note`, or `calendar`. Old `#...` docids are rejected with a native-cutover error.
...
- `Memory_Get` and `zdx memory get` read bounded indexed snapshots (≤ 40 KB / ≤ 1200 lines per read) and return source metadata plus truncation/range metadata; truncated reads continue from `next_start_byte` via the tool's `start_byte` input or `zdx memory get --start-byte`. Known thread IDs still use `Read_Thread`; known note/calendar paths still use direct file reads when current canonical content matters.
