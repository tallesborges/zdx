...
- The orchestrator's model and thinking level resolve as: the chat's configured model → `[subagents.overrides.orchestrator]` (`model = "provider:id@level"`, same shape as other subagent overrides; an override without an `@level` suffix keeps the chat's thinking level) → the topic's own `/model` and `/thinking` overrides. The status card reflects the same resolution. This is the only place the `orchestrator` override key takes effect, since the profile is never reachable through `invoke_subagent`.
