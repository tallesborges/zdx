...
- **Main config thinking precedence:** after config layers merge, an explicit `thinking_level` (including `off`, even inherited from a lower layer) wins. Only when that key is absent does the reader use the main `model`'s thinking suffix, otherwise defaulting to `off`. This reader-only compatibility step does not rewrite files or change model/thinking writers, helper-model resolution, or subagent overrides.
