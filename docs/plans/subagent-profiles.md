...
- **Scope checklist**:
  - [ ] `Profile` struct: `name`, `description`, `model` (Option), `thinking_level` (Option), `system_prompt` (String, from markdown body), `tools` (Option<Vec<String>>), `no_system_prompt` (bool)
...
- **`no_system_prompt` in profile**: when true, profile's markdown body is still used as the system prompt but AGENTS.md/memory/skills context composition is skipped
