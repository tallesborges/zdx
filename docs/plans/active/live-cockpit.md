...
- Gaps: ExampleCo + ExampleCo accounts must be added with `gog auth add` before multi-account works (Key decision).
...
- **Scope checklist**:
  ...
  - [ ] Response format: `[<anchor id>] <free-text intent>` (e.g. `[E-personal-abc123] archive this`, `[E-example-def456] draft a reply saying I'll review Monday`). Text stays natural language; the anchor makes the target unambiguous.
...
- **Goal**: `Important email` covers all three accounts (personal + ExampleCo + ExampleCo), tagged by source.
- **Scope checklist**:
  - [ ] Ensure `gog auth add` done for ExampleCo + ExampleCo (Key decision / prerequisite).
  - [ ] Loop the gmail search per account with `--account`; tag each item `[personal] / [example] / [example]`.
...
- **Multi-account prerequisite**: ExampleCo + ExampleCo must be added via `gog auth add` before Slice 3.
...
- **ExampleCo project actions** beyond email. Trigger: ExampleCo work actually starts.
