...
- **Scope checklist**:
  ...
  - [x] Enforce Telegram's entity-nesting rules, since `sanitize` cannot repair semantically invalid nesting: flatten nested blockquotes; lower inline code and links to plain text inside a continuous blockquote; render quoted code blocks outside the quote; do not nest formatting inside `code`/`pre`; do not nest links; suppress the outer entity (or close/reopen) for `**bold with `code`**` and `[text with `code`](url)` outside quotes.
