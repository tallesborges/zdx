...
- Changing bot-authored HTML (status message, pickers, launcher, keyboards, retry). Those are code-generated and already correct.
...
## Phase 1: Markdown-first replies (converter + prompt flip, shipped together)
...
- **Scope checklist**:
  - [ ] Add `crates/zdx-bot/src/telegram/markdown.rs` exposing `to_telegram_html(&str) -> String`, built on `pulldown_cmark::Parser::new_ext` with `Options::ENABLE_TABLES` only, matching the transcript renderer (`parse.rs:41-45`).
  - [ ] Add `pulldown-cmark.workspace = true` to `crates/zdx-bot/Cargo.toml`.
  - [ ] Map to Telegram's supported subset: `Strong`→`<b>`, `Emphasis`→`<i>`, `Code`→`<code>`, `Link`→`<a href>`, `BlockQuote`→`<blockquote>`, `CodeBlock`→`<pre><code class="language-x">…</code></pre>` (Telegram's documented form — never a language attribute on `<pre>`; extract and validate the first info-string token, omit on invalid/empty).
  - [ ] Enforce Telegram's entity-nesting rules, since `sanitize` cannot repair semantically invalid nesting: flatten nested blockquotes; do not nest formatting inside `code`/`pre`; do not nest links; suppress the outer entity (or close/reopen) for `**bold with `code`**` and `[text with `code`](url)`.
  - [ ] Lower what Telegram cannot render: headings → bold line; bullet/ordered items → flat `-` / `N.` lines with two-space-per-level indent for nesting (mirroring `parse.rs:759-760`); `Rule` → separator; tables → plain lines; images and unhandled events → explicit, documented behavior.
  - [ ] Define the whitespace contract explicitly and match the finalized transcript path: `SoftBreak` → space (`parse.rs:552-565`), `HardBreak` → newline, paragraph/block end → single blank line, with separator deduplication so blocks never accumulate 3+ newlines. Code payloads preserved byte-for-byte.
  - [ ] HTML-escape every `Event::Text`, every code payload, and every link `href` as an attribute value. Note pulldown already decodes source entities, so text is escaped exactly once.
  - [ ] Replace `normalize_reply_text` in the pipeline: feed post-control-tag text straight to the converter and let the converter own spacing. Collapsing blank lines before conversion corrupts fenced code; after conversion it corrupts `<pre>`.
  - [ ] Normalize followup button labels (strip inline-code backticks and emphasis markers) for display only, keeping the original value as the synthetic user message (`followups.rs:31-49`, `:136-139`). Required in this phase: once the prompt says "write Markdown", labels will contain backticks.
  ...
  - [ ] Rewrite `## Examples` (`:58-82`) in Markdown, including the closing "Avoid" line (`:82`) which currently forbids exactly what we now want.
  - [ ] Fix the intro (`:3`): "not a terminal, email, or Markdown renderer" is now false.
  - [ ] Fix the two HTML leaks inside `## Detailed answers and file uploads`: `<i>Full details attached ↓</i>` (`:47`) becomes Markdown emphasis, and "wrap it in `<code>`" (`:50`) becomes a backtick reference. The rest of that section (`:40-56`) and all of `## Suggested replies` (`:26-38`) stay as-is — the artifact rule and the `<followups>`/`<media>` control tags are unchanged.
  - [ ] Prompt states the exclusions: no tables in chat (use an artifact file), no `~~strikethrough~~` (renders unstyled in the TUI, `parse.rs:467-475`), and include bare URLs where the destination matters (the TUI drops link targets, `parse.rs:500-502`).
  - [ ] Unit tests in the `html.rs` style (inline `#[cfg(test)] mod tests`, behavior-named, exact `assert_eq!`) over fixed fixtures: each mapping; nested lists; nested quote; quote containing a code block; bold containing code; link containing code; stray `<` and `&` in prose; a Markdown code span containing a literal `&amp;`; blank lines inside fenced code; legacy HTML-authored replies captured from real threads; idempotency.
...
- **Risks / failure modes**:
  ...
  - The model may emit HTML out of habit during transition. Raw HTML is passed through and `sanitize` still governs it, so this degrades rather than breaks.
  - Prompt shrinks substantially; re-read the whole layer after editing to confirm no rule was orphaned.

## Phase 2: Model-text paths that bypass the choke point
...
- **Scope checklist**:
  - [ ] `/tldr`: the generator returns plain Markdown (`zdx-engine/src/core/tldr_generation.rs:17-25`) and the bot HTML-escapes it (`handlers/message/commands.rs:635-646`) — convert instead of escape.
  - [ ] `/handoff` and `/prompt_builder` previews: generators use no system prompt (`zdx-engine/src/core/handoff_generation.rs:159-176`, `prompt_builder_generation.rs:63-81`) and `suggestion_preview` escapes into a bot-authored `<blockquote>` (`staging.rs:756-777`, sent at `:332-364`) — convert instead of escape.
  - [ ] Drop the unconditional outer `<blockquote>` frame in `suggestion_preview` (`staging.rs:770-776`); converted content may already contain a quote and Telegram forbids nesting them. Use a title/separator frame, or flatten quotes inside previews.
  - [ ] Convert **before** truncating in both paths. Both currently truncate source Markdown first (`commands.rs:635-655`, `staging.rs:34-36`), which can cut a fence, link, or emphasis mid-construct. Truncate on visible characters without splitting tags or entities.
...
- **Converter and prompt flip ship together.** A parse/re-emit is not output-preserving for today's HTML-authored replies, so a "wire it inert first" split has no safety value. The alternative — a whole-message legacy-HTML detection bypass — is compatibility scaffolding this project's conventions reject.
