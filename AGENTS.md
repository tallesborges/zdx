...
- `crates/zdx-transcript/`: shared transcript display model + rendering (thread events → `HistoryCell`s → styled/ratatui lines; markdown, wrapping, tool pairing, `tool_detail_body` for full tool args/output). Reused by `zdx-tui` and `zdx-monitor`.
