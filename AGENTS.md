...
- `crates/zdx-transcript/`: shared transcript display model + rendering (thread events → `HistoryCell`s → styled/ratatui lines; markdown, wrapping, tool pairing, `tool_detail_body` for full tool args/output) plus shared TUI interaction utilities (`clipboard` with OSC 52 → arboard fallback, `keys::GPrefix` vim `gg` prefix). Reused by `zdx-tui` and `zdx-monitor`.
...
- `just deploy` (build release → install to `~/.local/bin/zdx` → guarded `zdx service restart all`; the standard way to ship a change to the running bot/daemon). The restart refuses to interrupt active agent runs; wait for them or explicitly run `zdx service restart all --force`. Set `ZDX_CODESIGN_ID` in your shell env to a code-signing identity so `install` signs the binary with a stable identifier and macOS TCC grants survive rebuilds; unset, the binary stays ad-hoc signed and macOS re-prompts after every build.
