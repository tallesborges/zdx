...
- `src/overlays/`: command palette, skill picker, rename overlays. Scrollable, non-text overlays (tool detail, TLDR, context, timeline, background) share the vim `GPrefix` helper in `overlays/mod.rs`: `gg` is delivered as `Home`, `G`/`End` jumps bottom, unknown `g` sequences are swallowed — same convention as the monitor. Overlays with a type-to-filter input (pickers, palette) must NOT use it.
