...
pub fn definition() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Search the web for current information and return ranked URLs with LLM-optimized excerpts. At least one of `objective` or `search_queries` is required. Use `objective` for the natural-language research goal and `search_queries` for concrete keyword searches; prefer providing both. Do not send a `query` field. `search_queries` must be a JSON array of complete query strings — each array item is one whole query. Put all keywords and search operators for a query inside the same string; never write bare tokens between strings. Prefer plain strings without exact-phrase quotes unless quotes are necessary. Example: {\"objective\":\"Find official docs for the Rust 2024 edition\",\"search_queries\":[\"Rust 2024 edition guide\",\"site:doc.rust-lang.org Rust 2024 edition\"],\"max_results\":5}.".to_string(),
        input_schema: json!({
            "type": "object",
            "properties": {
                "objective": {
                ...
                "search_queries": {
                    ...
                    "items": { "type": "string" },
                    "minItems": 1,
                    "description": "Keyword queries to supplement the objective. Each item must be one complete JSON string containing the full query, including any search operators such as site:github.com. Do not split one query into separate quoted and unquoted tokens. Prefer plain strings without exact-phrase quotes unless quotes are necessary. Providing both objective and search_queries yields best results. If you only have keywords, send them here instead of using a `query` field."
                },
                "max_results": {
                    "type": "integer",
...
mod tests {
    ...
    fn test_definition_schema() {
        ...
        assert!(
            sq_desc.contains("one complete JSON string"),
            "search_queries description should emphasize complete query strings: {sq_desc}"
        );
        assert!(
            sq_desc.contains("Do not split one query"),
            "search_queries description should warn against token-splitting: {sq_desc}"
        );
        assert!(
            def.description
                .contains("never write bare tokens between strings"),
            "tool description should warn against malformed JSON token sequences: {}",
            def.description
        );
