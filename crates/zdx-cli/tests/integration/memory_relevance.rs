...
fn build_corpus(temp_dir: &TempDir) {
    ...
    fs::write(
        notes.join("People").join("Jane Doe.md"),
        "# Jane Doe\n\nWeekly private English classes with Jane Doe.\n",
    )
    .unwrap();
    fs::write(
        notes.join("ExampleCo Links.md"),
        "# ExampleCo Links\n\nMan pages live at https://docs.example.com/trinity for the whole stack.\n\nAsk questions with `gh issue create --repo example-org/man` instead of PRs.\n\nStorage architecture is led by John Doe.\n",
    )
    .unwrap();
    fs::write(
        notes.join("Solar.md"),
        "# Solar\n\nSolar panel sizing for the home roof: around 5 kWp with inverter budget headroom.\n",
    )
    .unwrap();
    fs::write(
    ...
    fs::write(
        ...
        "# 2026-07-15\n\nDeployed identity backend to the staging server.\n",
    )
    .unwrap();
...
fn judged_queries() -> &'static [(
    ...
    &[
        ...
        (
            "url",
            "https://docs.example.com/trinity",
            None,
            "note://ExampleCo Links.md",
            1,
        ),
        (
            "error-string",
        ...
        (
            "command",
            "gh issue create --repo example-org/man",
            None,
            "note://ExampleCo Links.md",
            1,
        ),
