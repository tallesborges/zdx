...
pub async fn login_antigravity() -> Result<()> {
    ...
    let pkce = oauth_antigravity::generate_pkce();
    let state = uuid::Uuid::new_v4().to_string();
    let auth_url = oauth_antigravity::build_auth_url(&pkce, &state);

    println!("To log in to Google Antigravity with Google OAuth:");
    println!();
    println!("  1. A browser window will open (or visit the URL below)");
    println!("  2. Log in with your Google account and authorize access");
    println!("  3. If redirected to localhost, return here to continue");
    ...
    let code = if let Some(code) = wait_for_antigravity_code(&state) {
        code
    } else {
        print!("Paste authorization code (or full redirect URL): ");
        io::stdout().flush()?;
        let mut input = String::new();
        io::stdin().lock().read_line(&mut input)?;
        let (code, provided_state) = oauth_antigravity::parse_authorization_input(&input);
        if let Some(provided) = provided_state
            && provided != state
        {
            anyhow::bail!("State mismatch");
    ...
    println!("Exchanging code for tokens...");
    let mut credentials = oauth_antigravity::exchange_code(&code, &pkce).await?;

    println!("Discovering Antigravity project...");
...
fn wait_for_antigravity_code(state: &str) -> Option<String> {
    let Ok(listener) = TcpListener::bind("127.0.0.1:51121") else {
        return None;
    };
        ...
        loop {
            match listener.accept() {
                Ok((mut stream, _)) => {
                    ...
                    let code = extract_antigravity_code_from_request(&request, &state);
...
fn extract_antigravity_code_from_request(request: &str, expected_state: &str) -> Option<String> {
