...
- `src/server.rs`: embedded HTTP web server for the Threads and Monitor Mini Apps. `/threads` and `/monitor` serve embedded pages; every `/api/*` endpoint requires fresh bot-token-signed Telegram `initData` from an allowlisted user. The read-only monitor snapshot is built off the async hot path and cached for 30 seconds; live quotas come from the same shared snapshot API as `zdx quota`.
- `src/miniapp.html`: Threads Mini App frontend.
- `src/monitor.html`: mobile-first, read-only Monitor Mini App frontend for services, active/background work, live subscription quotas, 30-day usage, safe config, and automations. Subscription endpoints are read-only and cached for five minutes.
