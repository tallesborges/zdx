...
- `src/orchestrator.rs`: orchestrator worker bridge — consumes engine `WorkerManager` `WorkerEvent`s: `Created` opens a mirror topic (aliased + `worker_topic`-marked) in the project's/owner's forum chat and registers its `t.me/c/...` link as the worker's `mirror_url` (resolved to `None` when no mirror opened, so `create_thread` never waits out its bounded link wait); `Activity` maintains one debounced live `⏳ Working…` message per running turn (tool call list, bounded window); `Completed` freezes that message, posts the result and wakes the owning orchestrator topic with a bounded synthetic `[worker update]` turn. Mirror header/activity/result messages carry `⏹ Cancel worker` (`wk:c`, worker resolved from the topic's alias) and Mini App `💬 Open Thread` buttons. At startup the bridge rebuilds the worker→topic map from persisted `worker_topic` + `alias_to` meta (`list_worker_topics`) so feeds resume after restarts; routes stay process-lifetime
