...
- `src/bot/context.rs`: shared bot context; also owns per-profile layered configs (`config_for_chat`), the process-lifetime orchestrator route map (owner thread → chat/topic/user for worker completion callbacks), the queued-restart slot, and the thread → pinned-header message map
...
- `src/handlers/message/commands.rs`: slash-command handlers (`/new`, `/model`, `/status`, `/whereami`, `/launcher`, thread/worktree, exit, `/restart` incl. the `q` mode's background wait-for-idle task that polls `agent_activity::list_active` and fires the gated restart, single-slot via `BotContext::try_claim_queued_restart`) + the provider → model → thinking picker + `ModelPickerScope` (General/Topic/NewThread)
- `src/handlers/message/launcher.rs`: General-topic thread launcher — bot-visible `[[model_modes]]` filter, `create_topic_with_model`, `create_topic_resuming`, `/launcher` keyboard (`nt:p:{mode}`/`nt:custom`/`nt:resume`) + callback routing; Custom opens the model picker in `NewThread` scope; `🔄 Continue` picker resumes a source thread via `alias_to`; `LauncherMap` + `schedule_repost` keep the launcher as the last message in General (debounced per-chat repost)
...
- `src/handlers/message/thread_header.rs`: first-message topic status card, Mini App/refresh keyboard, and silent pinning; records the posted header per thread on `BotContext` so `refresh_thread_header` can re-render it in place (the worker bridge does this on worker events for orchestrator cards)
- `src/handlers/message/response.rs`: final response sending (text send/edit/fallback). Every answer gets a trailing `↗ Open thread` Mini App deep link (`{mini_app_url}?startapp={thread_id}`) via `append_thread_link`, skipped when the Mini App is not configured for the chat; orchestrator answers also get one `🛠 <title>` mirror-topic link per worker the turn created or messaged (`TurnResult::touched_workers`, collected from `Create_Thread`/`Send_Thread_Message` results in `turn.rs`). Answers are sent through the `*_without_preview` client wrappers so that link does not render a preview card; other message paths keep previews.
...
- `src/agent/mod.rs`: thread log + agent turn helpers; orchestrator prompt extras (`telegram_workspaces_block` lists bound profiles plus each root's project skills via `workspace_skills`, the `$ZDX_HOME/orchestrator.md` overlay)
