...
fn build_description(subagents: &[SubagentSummary]) -> String {
    let mut description = "Delegate a scoped task to an isolated child agent run. Best for complex multi-step work, output-heavy subtasks, independent parallel implementation slices, or broad discovery that would clutter the main context. Prefer doing the work directly when it is small enough to complete without delegation. Child runs are self-contained and do not share your full parent reasoning or implicit context, so every important decision, relevant detail, file path, constraint, non-goal, and acceptance criterion must be made explicit in the prompt. Provide a focused prompt with the goal, relevant context, constraints/non-goals, file paths, expected output, and how success should be verified. NEVER delegate trivial reads, searches, or edits you can do directly. Trust but verify: if a child reports edits or claims you plan to rely on, inspect the resulting files or key evidence before reporting success. Use `subagent` to select a named configuration, or `task` for the default delegated ZDX behavior when no named specialist fits. Returns response text only. Skill names are invalid unless they are also listed as supported subagents.".to_string();
