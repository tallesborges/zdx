...
pub fn definition() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Answer a goal using a saved thread transcript. Use Thread_Search first to find the thread_id — do not guess or invent thread IDs. Provide a specific `goal` describing what to extract; vague goals return vague answers. Best for historical thread context, prior decisions, or past outputs rather than current filesystem state. Returns response text only."
            .to_string(),
        input_schema: json!({
            "type": "object",
            "properties": {
