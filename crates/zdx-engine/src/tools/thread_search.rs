...
pub fn definition() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Search through saved ZDX conversation threads by content, keywords, file references, dates, or thread titles. Use this to find thread IDs before calling Read_Thread — never guess a thread_id. Prefer query/date filters instead of opening many threads blindly. Supports query/date filters and returns structured thread matches including thread IDs, titles, and activity dates.".to_string(),
        input_schema: json!({
            "type": "object",
            "properties": {
                "query": {
