...
pub fn definition() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Search saved ZDX memory in the native SQLite index across Notes, Calendar, and exported conversation threads. Each hit returns the `path` it was indexed from, plus `thread_id` for threads. Read the best few before rephrasing the query: `Read` the path for notes and calendar, and `Read_Thread` the thread_id for threads, whose path is an export that only refreshes when the index runs. Set `source` to target one collection — use `note` for the user's Notes/Second Brain. To find a past conversation prefer Thread_Search; use `source: \"thread\"` here only to search threads alongside the other collections. Omit `strategy` for lexical search; `vector` and `hybrid` require a configured embedding profile and fail clearly without one. `intent` is not a filter and applies only to vector/hybrid. When the thread_id is known, call Read_Thread directly."
            .to_string(),
        input_schema: json!({
            "type": "object",
            "properties": {
